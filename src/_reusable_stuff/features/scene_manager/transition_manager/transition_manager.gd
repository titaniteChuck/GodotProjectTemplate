...
func _init_loaded_scene(caller_node: Node, data: SceneManager_TransitionData) -> Node:
	...
	if data.init_method:
		if caller_node.has_method(data.init_method) :
			for method in caller_node.get_method_list():
				if method.name == data.init_method:
					if method.args.size() != 1:
						assert(false, "Transition_Manager.init_method: wrong argument count for [%s], should have the signature func(%s)" % [data.init_method, node_to_load.get_class()] ) 
					elif not node_to_load.is_class(method.args[0].class_name):
						assert(false, "Transition_Manager.init_method: Wrong expected type [%s(%s)], should be func(%s)" % [data.init_method, method.args[0].class_name, node_to_load.get_class()] ) 
					else:
						caller_node.call(data.init_method, node_to_load)
					break;
		else:
			assert(false, "Transition_Manager.init_method: [%s] not found in [%s]" % [data.init_method, caller_node.get_class()] )
	return node_to_load
#endregion
